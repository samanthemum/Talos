...
namespace vkInit {
	...
	std::vector<std::string> logTransformBit(vk::SurfaceTransformFlagsKHR bits) {
...
This mode may result in visible tearing if rendering to the image is not timed correctly.";
	...
	void logDeviceProperties(const vk::PhysicalDevice& device) {
		...
		std::cout << "\tDevice version: " << props.apiVersion << std::endl;
		switch (props.deviceType) {
		case vk::PhysicalDeviceType::eDiscreteGpu:
			std::cout << "\tDevice Type: Discrete GPU";
			break;
		default:
			std::cout << "\tDevice Type: Other";
		}
	}
}